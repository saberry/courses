...
This is the most automated way of finding a potentially suitable number of factors. If some theory exists, you can also use that information to guide your factor numbers. 
...
We can see that our output is hiding values of small magnitude (refer to the previous results for all values). We can read these very much in the same way that we read correlations. For factor 1 (MR1 in the output), we see that items C1 through C5 load pretty strongly (with A4 having a weak loading). So, someone who offers a high response on C5 might have lower values of whatever latent trait is being measured, while they might have high values of the latent trait if they respond with a higher value on question C2.
...
To compute those score, we need to produce a correlation matrix of our observed scores. For simplicity, let see what a one factor model would look like.
...
psych::omega(bfiSubFactors, nfactors = 5, rotate = "promax")
