...
In IRT, we talk a person's ability; essentially it is the level of the latent variable that possess. In the parlance of psychometrics, ability is denoted as $\theta$. The $\theta$ scale is standardized with a mean of 0 and tends to go from -4 to 4 (while this is generally seen, it might not always be the case). 
...
Let's look at some examples from the *ltm* documentation of different item locations.

```{r}
library(ltm)

fit <- rasch(LSAT, constraint = cbind(ncol(LSAT) + 1, 1))
...
Let's take a look at our ICC again:
...
With some minor differences, you will often see the 1PL and Rasch model used interchangibly.
...
grmMod = ltm::grm(testData)

summary(grmMod)

```

In our summary output, we see coefficients for each item (C1:C5). These coefficients detail the *thresholds* for moving from one response level to the next (we also see the discrimination value). 

Just like our dichotomous models, we can also see our item characteristic curves and an item information curve.

```{r, fig.height=9, fig.width=9}

par(mfrow = c(3, 2))

plot(grmMod, type = "ICC")

plot(grmMod, type = "IIC")
```
...
We can also get item operation characteristic curves:

```{r, fig.height=9, fig.width=9}
par(mfrow = c(3, 2))

plot(grmMod, type = "OCCu")
```
