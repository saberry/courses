...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, comment = "", results = )
```
...
While this is certainly a contrived example and would need addition robustness testing, it goes to show you that working with interesting data will yield interesting problems. I have been telling you all along that nothing is ever as it seems.
...
textPrepFunctions = list(content_transformer(tolower), 
                         removePunctuation,
                         stemDocument,
                         stopWordRemoval,
                         removeNumbers,
...
Once you get your text tidied up, you can produce some visualizations!
...
load("N:/R/texasexecutions/Data_Files/Working/RData/txExecutionsPublish_20150401.RData")
...
Sentiment analysis is commonly used when we want to know the general *feelings* of what someone has written. Sentiment analysis is commonly seen applied to Twitter and other social media posts, but we can use it anywhere where people have written/said something (product reviews, song lyrics, final statements).
...
We will use the *vader* sentiment library, but many more available. Depending on your exact needs, there are some dictionaries designed for different applications. A prime example, and one that should be near to our hearts, is the Loughran and McDonald dictionary for financial documents (e.g., SEC 10K filings). Tim Loughran and Bill McDonald are superstars in the Finance Department at Notre Dame!

Before we engage in our whole sentiment analysis, let's take a look at a few things.

Here is the dictionary that *vader* will use.
...
statementSentiment = sentiment(get_sentences(txExecutions), 
                               polarity_dt = lexicon::hash_sentiment_vadar)
```
...
statementTokens = tokenize(statement, format = "obj", lang = "en")
...
```{r}
readability(statementTokens)
```

```{r}
lex.div(statementTokens)
```


## Do Me A Favor

There are some hushed words about employer reviews on the web. Why don't you take a look at them and see what you can find out. 