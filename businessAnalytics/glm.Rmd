...
$$ F = \frac {(\Sigma(y - \hat{y})^2)/v-1} {(\Sigma(\hat{y}-\bar{y})^2)/n-v} $$

We already know most of this, the top part has the explained sums of squares and the bottom part is the residual sums of squares. *v* is the number of variables and *n* is the sample size and those terms represent the degrees of freedom.

<aside>
Just remember that the degrees of freedom is almost always going to start with the number of observations and then take away the number of parameters that need to be estimated from the population.
</aside>

```{r}
fStat = (explainedSS / 1) / (residualSS / (nrow(xyDat) - 2))
```
