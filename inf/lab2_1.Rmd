...
```{r}
link <- "http://www3.nd.edu/~sberry5/class/anncomp.csv"
```
...
names(cpiTable) <- cpiTable[1, ]

cpiTable <- cpiTable[-c(1), ]
```

Now we can join the cpiTable into our compensation data and create an inflation-adjusted total salary variable.
