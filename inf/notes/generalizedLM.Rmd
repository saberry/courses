...
```{r}
read.csv("https://www.nd.edu/~sberry5/data/breakdowns.csv")
...
To put some numbers to that, if we have 1 accident per week in a factory and we are observing a whole year, we would get the following:

```{r}
(1 / 7) * 365
```

We expect about 52 accidents over the course of the year.

Let's see what that particular distribution might look like:

```{r}
plot(density(dpois(1:52, 52)))
```
...
$$Poisson(\lambda) = \alpha + \beta X \\ log() = $$
