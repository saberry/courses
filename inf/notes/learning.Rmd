...
Not that we will cover it in here, but the "RCurl" package is most excellent for making API calls, among other things.


## Cross Validation

```{r}
library(caret); library(dplyr); library(gam)

modVars = creditScores %>% 
  dplyr::select(Savings, Income, Age)

car::scatterplotMatrix(modVars)

trainingPartition = createDataPartition(modVars$Savings, 
                                        p = .85, list = FALSE)

trainingData = modVars[trainingPartition, ]

testingData = modVars[-trainingPartition, ]
...
predictedNew = predict(cvLM, newdata = testingData)

postResample(pred = predictedNew, obs = testingData$Savings)
```
...
Another great thing about these trees is the ability to use classification (think logistic regression) or regression.
...
If we notice the "extra" argument in our prp function, we will see an 8.  This option is giving us the probability of belonging to that class (e.g., for everyone with a savings over 2, there is a .78 percent chance of them having good credit).  There are many options, so be sure to look at the help file for the different options.  
...
For the sake of simplicity, we have been dealing with pretty sterile data in class.  Unfortunately, not all data is clean and missingness and statistics do not really hang out with each other (i.e., most statistics will drop rows with missing data or fail).  There are many ways to impute data, ranging from mean/median imputation (don't do this) to multiple imputation (e.g. MICE).  You could also impute missing data with such a decision tree.

## Random Forest

Decision trees are clearly a lot of fun and have a wide variety of uses.  While sometimes one tree is helpful, everything I know about horror movies leads me to believe that a single tree (perhaps on a desolate hill) always leads to some type of danger.  If we remember the issue with trees being greedy and just splitting things how it finds it best.  

This brings us to ensemble methods.  If we think about what an ensemble is, we can start to imagine what ensemble methods might be.  There are many ensemble methods, but the one we are going to talk about now is random forests.  Trees...forest...ensemble...are the connections starting to happen?
...
Now we know what variables are important for our random forest, so when do we interpret our trees!?!  We can't...so we don't!!! Interpreting a single tree is statistically non-sense when we are dealing with a random forest.  We are really only concerned with prediction at this point.
...
What happened?  While we do not want to overgrow a tree, a random forest does not care in the slightest.  In fact, it will grow the deepest tree that it possible can.  If you would say that this causes over-fitting, then you would be absolutely correct.  Remember, though, that we are generating a lot of trees and using all of these trees to votes on the outcomes.  Therefore, the over-fitting within one tree really does not make a big differenece -- it will come out in the wash.  But, you can see where interpretting a tree from a forest is practically nonsense!
