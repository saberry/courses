...
performanceReview <- readr::read_csv("inf/performanceReviewData.csv")
...
```{r}
ggplot(performanceReview, aes(as.factor(idDepartment), performanceScore)) +
  geom_boxplot() + 
...
We also see some additional information -- this is for our random effects. The standard deviation is telling us how much the score moves around based upon department after getting the information from our fixed effects (i.e., the score can move around nearly 2 whole point from department alone). We can also use various bits of information in our output to create different *R^2* values. 
