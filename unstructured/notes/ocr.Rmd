...
R has many classic packages, with `e1071` being one of them. For our work in caret, will also play with `kernlab`.
...
We see that we have a tuning parameter called `C`. Remember the planes that we drew earlier, where the goal was to correctly classify and maximize the margins? We cannot always do both in reality and the `C` parameter indicates which we prefer. Higher C values will push us towards *c*orrect classification over maximizing the margins. It essentially serves as a regularization parameter. The values that the `C` parameter can take can be very small to very large
...
That is going to be a nope for me. We are dealing with only 3 dimensions (out of several more) and I am not entirely convinced that we could find linear lines to do that splitting for us.

Instead, we can do some kernel transformations for the SVD. This might cause better separation in our data, thus lending to better predictions. We will also add in a new tuning parameter: sigma. Sigma is going to control the smoothness of the decision boundary. Since we are dealing in nonlinear space, the decision boundary can be very choppy (low values of sigma and likely to overfit -- a local classifier) or very smooth (higher values of sigma and more likely to produce training errors -- a global classifier).

```{r}
tuningGrid = expand.grid(C = c(1, 5, 10), 
                         sigma = c(.5, 1, 5, 10))

svmLetterLinear = train(lettr ~ ., data = letterTrain, method = "svmRadial", 
                        trControl = svmTrainControl, metric = "Accuracy", 
                        preProcess = c("center", "scale"), 
                        tuneGrid = tuningGrid)
