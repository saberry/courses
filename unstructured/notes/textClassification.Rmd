...
scaledVars = as.data.frame(scale(textFeatures[, 2:length(names(textFeatures))]))

scaledData = cbind(genre, scaledVars)

set.seed(1001)

trainRows = c(sample(zappaRows, floor(length(zappaRows) * .8), replace = FALSE), 
              sample(countryRows, floor(length(countryRows) * .2), replace = FALSE))

trainingData = scaledData[trainRows, ]

testingData = scaledData[-c(trainRows), ]
```
...
oneLayerNN = nnet(genre ~ ., data = scaledData, subset = trainRows, size = 2, 
                  MaxNWts = 13 * (ncol(scaledVars) + 1) + 13 + 1, 
                  maxit = 500)
```
That should have moved pretty quickly.

Now, we need to start looking at how we did on the training data:

```{r}
temporaryTestData = select(trainingData, -genre)

results = compute(oneLayerNN, temporaryTestData)
```
