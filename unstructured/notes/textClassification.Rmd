...
Classificiation is nothing new at this point -- you have seen methods ranging from simple logistic regression to random forest and beyond. We are going to see two alternative methods for classification: Naive Bayes and neural networks. Both of these techniques can offer reasonable prediction, without too much complexity. 
...
We will step away from text just for a little bit to see a good example of how Naive Bayes can be put to great use. The `rsample` package is great for creating data for us in cross-validation. 
...
attrition <-  attrition %>%
  mutate(across(c("JobLevel", "StockOptionLevel", "TrainingTimesLastYear"), ~ as.factor(.x)))
...
With the class split in mind, we need to develop a training set that will represent the proportion of country and zappa. We would find that the zappa class represents `r 95 / (189+95)`% of the total data. 
...
searchGrid <- expand.grid(usekernel = c(TRUE, FALSE), 
                           laplace = 1:2, adjust = seq(1, 3, by = 1))

nbTrainControl <- trainControl(method = "cv", number = 10, verboseIter = FALSE)

nbAttritionTuned <- train(x = x, y = y,
  method = "naive_bayes", trControl = nbTrainControl,
  tuneGrid = searchGrid)
...
testNormalized <- bake(genreRec, new_data = genreTest, 
...
We can try some boosted trees now:
...
xgb_grid <- grid_latin_hypercube( # not as silly as a full grid search!
  tree_depth(),
  min_n(),
  loss_reduction(),
  sample_size = sample_prop(),
  finalize(mtry(), genreTrain),
  learn_rate(),
  size = 30
)
...
genre_folds <- vfold_cv(genreTrain, strata = genre)

xgb_res <- tune_grid(
  xgb_wf,
  resamples = genre_folds,
  grid = xgb_grid,
...
xgb_recipe <- recipe(genre ~ ., data = genreTrain)

X <- prep(xgb_recipe, genreTrain) %>% 
  juice() %>% 
  dplyr::select(-genre) %>%
  as.matrix()
...
colnames(X) <- xg_mod$fit$feature_names

shap <- fastshap::explain(object = xg_mod$fit,  
                          newdata = X, exact = TRUE)

autoplot(shap, type = "contribution", row_num = 2, num_features = 20)
```


If you want to take a detour from the RStudio folks, you can try out `mlr3`. It is part of a growing set of R packages that use an R6 class of objects. For my money, it is far more straightforward than the tidymodels framework.

```{r}
# remotes::install_github("mlr-org/mlr3extralearners")
# mlr3extralearners::install_learners("classif.nnet")
library(mlr3)
library(mlr3viz)
...
names(textFeatures) <- make.names(names(textFeatures))

genreClass <- TaskClassif$new(id = "zappa", 
                             backend = textFeatures, 
                             target = "genre", 
                             positive = "X1")

learner <- lrn("classif.nnet")

learner$param_set$values = list(size = 3)

resampling = rsmp("cv")

resampling$instantiate(genreClass)

resampling$iters

rr <- resample(genreClass, learner, resampling, store_models = TRUE)
