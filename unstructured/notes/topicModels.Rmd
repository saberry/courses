...
How does this dimension reduction happen? It happens with singular value decomposition (SVD). SVD will take a matrix (like the one we have) and break it down into 3 component matrices: $M=U*S*V$. The *S* is where SVD gets its name because it is a diagonal matrix of the singular values of *M*. 
...
executions = read.csv("https://www.nd.edu/~sberry5/data/txEx.csv", stringsAsFactors = FALSE)

executions = executions %>% 
  select(correctedStatements, totalVictims, codefsYes) %>% 
  na.omit()
...
Let's focus on the frex words (they occur *fr*equently within the topic and are *ex*clusive to that topic) and the highest probability words (i.e., the words that have the highest probability of occuring within that topic). The Lift and Score words (just a few different ways of weighting occurance) can be useful, but are a bit less intuitive than the other two. Let's put some names to the topics. Topic 1 is religious in nature, topic 2 is something along the lines of "everything is going to be alright and take care", topic 3 is "you are murdering an innocent man", topic 4 is probably "I hope your family finds peace", and topic 5 is "I hope everyone forgives me".
...
Document 1, for example has a probability of ~.91 for belonging to topic 1. Document 9 has a probability of .30 of being related to topic 2 and .38 for topic 5.

```{r}
executionsPrep$meta$correctedStatements[1]

executionsPrep$meta$correctedStatements[9]
```

We can also see what terms are in documents 1:

```{r}
executionsPrep$documents[[1]]
```

```{r}
executionsPrep$vocab[executionsPrep$documents[[1]][1, ]]
```
...
```{r}
executionsPrep$documents[[9]]
```

```{r}
executionsPrep$vocab[executionsPrep$documents[[9]][1, ]]
```
