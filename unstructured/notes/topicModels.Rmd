...
hardRemove = c(78, 91, 136, 213, 222, 232, 239, 245, 248, 268, 277, 285, 291, 
               309, 419, 424, 425, 460, 467, 482, 490, 519, 532, 545, 555, 654, 
               658, 682, 705, 719, 722, 780, 794, 795, 802, 836, 851, 878, 891, 
               921, 924, 972, 985, 995, 1026, 1041, 1101, 1102, 1111, 1144, 1145, 
               1158, 1166, 1172, 1186, 1202, 1204, 1209, 1215, 1257, 1287, 1311, 
               1316, 1326, 1342, 1361, 1493, 1534, 1584, 1609, 1629, 1649, 1667, 
               1676, 1679, 1707, 1710, 1720, 1755, 1760, 1798, 1805, 1855, 1861, 
               1918, 1929, 1994, 2000, 2011, 2017, 2018, 2027, 2039, 2055, 2164, 
               2200, 2254, 2285, 2303, 2334, 2373, 2397, 2401, 2406, 2434, 2469, 
               2475, 2499, 2507, 2527, 2534, 2540, 2566, 2570, 2655, 2724, 2729, 
               2736, 2764, 2767, 2771, 2803, 2843, 2877, 2905, 2913, 2944, 2951, 
               2972, 2995, 3043, 3092)
...
cleanLyrics = allLyricsDF[-c(hardRemove)] %>%
  ...
  mutate(text = as.character(lyrics), 
         ...
         text = str_replace_all(text, "(\\[.*?\\])", ""), # look different?
...
Now we can do some exploration, like which words are associated with they word "baby"?

```{r}
associate(lyricsSpace, "baby")
```
...
- We will start with our original term-document matrix (*O*). We know that is 9118 rows (words) by 2402 columns (songs). 

- We can then choose a number of topics (*k* = 15). This will give us a features matrix (*F* -- typically denoted as *W*) that is 9118 rows by 15 columns. It will also give us a coefficient matrix (*C* -- typically denoted as *H*) that is 15 rows by 2402 columns.
...
- When taking $FC$, we get a matrix that is 9118 by 2402 (our original dimensions). 

Since our data is already prepped, we can just pass our TDM right into the `nmf` function...in theory.

In reality, though, we need to add a small constant to our matrix. Why? The *k*-means that is being implemented on the backend of `nmf` will have some issues with all of those 0's; in other words, it will handle a smaller sparse matrix, not a bigger sparse matrix. It is not big deal
...
**Warning!** This takes a really long time to run! The clustering that is happening within the matrices is a pain -- if you have ever messed around with cluster selection, this will be familiar. Run it with smaller numbers of *k* to have it finish in decent time; the saved model object is in the data folder. 
...
We can find all of the words that fit within a topic (the *features*) based upon a threshold.
...
Non-negative matrix factorization is a useful tool that we will see again (it is great for working with images). For topic models, though, it has been surplanted by latent dirichlet allocation (LDA)

Let's start with a brief demonstration of a standard latent dirichlet allocation (LDA) for topic modeling. ics via LDA.
...
Topic models can be extended to include covariates. If we add a variable to the prevalence argument, we are examining how much each topic contributes to a document given some other data.
...
With our new model, we can actually test the effect of totalVictims on topic.
...
We can add additional information to our models: content. What we get here is a better understand about how people use language within a topic differently.
...
With this new model, we can see how people without and with codefendents use language within a topic:
